...
- (void)initTitleView
{
    ...
    titleLab.textColor = kRGBColor(62, 124, 254);
...
- (void)createBackBtn
{
    ...
    [btn setFrame:CGRectMake(15, 30, 25, 25)];
    [btn setImage:[UIImage imageNamed:@"back_blue"] forState:UIControlStateNormal];
...
{
    return CGRectMake(kLineMinY / kScreenHeight, ((kScreenWidth - asize.width) / 2.0) / kScreenWidth, asize.height / kScreenHeight, asize.width / kScreenWidth);
}

- (void)setOverlayPickerView
{
    //画中间的基准线
    _line = [[UIImageView alloc] initWithFrame:CGRectMake((kScreenWidth - 300) / 2.0, kLineMinY, 300, 12 * 300 / 320.0)];
    ...
    CGFloat space_h = kScreenHeight - kLineMaxY;
    
    //底部view
    UIView *downView = [[UIView alloc] initWithFrame:CGRectMake(0, kLineMaxY, kScreenWidth, space_h)];
    downView.alpha = 0.3;
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (metadataObjects.count > 0)
    {
        ...
        if (obj.stringValue && ![obj.stringValue isEqualToString:@""] && obj.stringValue.length > 0)
        {
            ...
            if ([obj.stringValue containsString:@"jnzddevqrcode-com.gcp0534://"])
            {
